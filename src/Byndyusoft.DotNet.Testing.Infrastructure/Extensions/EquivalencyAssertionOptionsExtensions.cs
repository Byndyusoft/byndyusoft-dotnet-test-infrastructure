namespace Byndyusoft.DotNet.Testing.Infrastructure.Extensions
{
    ...
    public static class EquivalencyAssertionOptionsExtensions
    {
        ...
        /// <summary>
        ///     Проверяет, что в проверяемой сущности все значения типа DateTime или DateTimeOffset находятся в пределах
        ///     определённого интервала от заданного момента времени
        /// </summary>
            ...
            TimeSpan precision)
        {
            return options
                   ...
                   .WhenTypeIs<DateTime>()
                   .Using<DateTimeOffset>(context => context.Subject.Should().BeCloseTo(context.Expectation, precision))
