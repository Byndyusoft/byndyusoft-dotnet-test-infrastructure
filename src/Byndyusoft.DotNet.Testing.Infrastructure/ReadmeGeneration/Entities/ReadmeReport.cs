...
internal sealed class ReadmeReport
{
    ...
    public ReportErrors GetErrors()
    {
        ...
        var duplicateTestIds = testCases.GroupBy(testCase => testCase.TestId)
                                        .Where(sameTestIdGroup => string.IsNullOrWhiteSpace(sameTestIdGroup.Key) == false)
                                        .Where(sameTestIdGroup => sameTestIdGroup.Count() > 1)
                                        .ToDictionary(sameTestIdGroup => sameTestIdGroup.Key!, 
                                                      sameTestIdGroup => sameTestIdGroup.ToArray());
